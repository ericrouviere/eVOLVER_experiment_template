...
class EvolverNamespace(BaseNamespace):
    ...
    def transform_data(self, data, vials, od_cal, temp_cal):
        ...
        for x in vials:
            ...
            # Eric's addition
            raw_OD_lower = od_coefficients[0]  
            raw_OD_upper = od_coefficients[1]
            
            try:
                if od_cal['type'] == SIGMOID:
                    
                    # Eric's addition to avoid returning nans
                    if od_data[x] >= raw_OD_upper:
                        od_data[x] = 1 # arbitrary large value
                    elif od_data[x] <= raw_OD_lower:
                        od_data[x] = 0
                    else:    
